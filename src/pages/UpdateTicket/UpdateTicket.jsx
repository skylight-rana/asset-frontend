...
function UpdateTicket() {
  ...
  const loadTickets = async () => {
    try {
      ...
      const [ticketRes, employeeRes, assetRes] = await Promise.all([
        getTickets(),
        getEmployees(),
        getAssets(),
      ]);
      ...
      setNotification({
        ...
        message: getApiErrorMessage(error, "Failed to load tickets."),
      });
    } finally {
      setLoading(false);
    }
  ...
  const getAssignableEmployees = (ticket) => {
    return employees.filter(
      (employee) => String(employee.id) !== String(ticket.employeeId)
    );
  };

  const getCurrentUserId = () => {
    return currentUser?.userId || currentUser?.UserId || null;
  };
  ...
  const getCurrentUserProfilePhoto = () => {
    return currentUser?.profilePhoto || currentUser?.ProfilePhoto || "";
  };
  ...
  const saveTicketUpdate = async (ticket, changes, successMessage) => {
    try {
      ...
      const payload = {
        ...
        resolutionNotes:
          changes.resolutionNotes ?? ticket.resolutionNotes ?? "",
        assignedToEmployeeId:
          changes.assignedToEmployeeId ?? ticket.assignedToEmployeeId ?? null,
        assignedByUserId:
          changes.assignedByUserId ?? ticket.assignedByUserId ?? null,
      };
      ...
      setNotification({
        ...
        message:
          successMessage || `Ticket #${ticket.id} updated successfully.`,
      });
    } catch (error) {
      setNotification({
        type: "error",
  ...
  const handleAssigneeChange = async (ticket, assignedToEmployeeId) => {
    const nextAssignedToEmployeeId = assignedToEmployeeId
      ? Number(assignedToEmployeeId)
      : null;

    if (
      String(nextAssignedToEmployeeId || "") ===
      String(ticket.assignedToEmployeeId || "")
    ) {
      return;
    }
    ...
    if (
      ...
      String(nextAssignedToEmployeeId) === String(ticket.employeeId)
    ) {
      setNotification({
        type: "error",
        title: "Invalid assignment",
        message:
          "You cannot assign a ticket to the same employee who raised it.",
      });
    ...
    const assignedEmployee = employees.find(
      (employee) => String(employee.id) === String(nextAssignedToEmployeeId)
    );

    const assignedByUserId =
      getCurrentUserId() || ticket.assignedByUserId || null;
    ...
    try {
        ...
        previousTickets.map((item) =>
          item.id === ticket.id
            ? {
                ...
                assignedByUserId,
                assignedByUserName: getCurrentUserName(),
                assignedByUserEmail: getCurrentUserEmail(),
                assignedByUserProfilePhoto: getCurrentUserProfilePhoto(),
              }
            : item
        )
      );
      ...
      setNotification({
        ...
        message: getApiErrorMessage(
          error,
          "Ticket assignee could not be updated."
        ),
      });
    } finally {
      setUpdatingTicketId(null);
  ...
  const showAsset = (ticket, e) => {
    ...
    setSelectedAsset(
      assets.find((asset) => String(asset.id) === String(ticket.assetId)) || {
        id: ticket.assetId,
  ...
  const showEmployee = (ticket, e) => {
    ...
    const employee = employees.find(
      (item) => String(item.id) === String(ticket.employeeId)
    );
    ...
    <DashboardLayout role="Admin" title="Tickets">
                  ...
                  {paginatedTickets.map((ticket) => (
                    <tr key={ticket.id}>
                      ...
                      <td
                        ...
                        onClick={(e) => showEmployee(ticket, e)}
                      >
                        {getTicketEmployeeName(ticket, employees)}
                      </td>

                      <td
                        className="clickable-text"
                        onClick={(e) => showAsset(ticket, e)}
                      >
                        {ticket.assetName || `Asset #${ticket.assetId}`}
                      </td>
                      ...
                      <td>
                        <select
                          ...
                          value={ticket.assignedToEmployeeId || ""}
                        ...
                        >
                          ...
                          {getAssignableEmployees(ticket).map((employee) => (
      ...
      >
        <div className="profile-detail-card">
          ...
          <div>
            <h3>{selectedEmployee?.name || "N/A"}</h3>
        ...
        <div className="detail-grid">
          <div className="detail-item">
            ...
            <span className="detail-value">
              {selectedEmployee?.id ? `#${selectedEmployee.id}` : "N/A"}
            </span>
          </div>
